...
def run(config):
    ...
    replay_buffer = ReplayBuffer(config["buffer_length"], model.nagents,
                                 [91 for _ in range(env.nagent)],
                                 [env.action_space for _ in range(env.nagent)])
    t = 0
    for ep_i in range(0, config["n_episodes"], config["n_rollout_threads"]):
        ...
        for step in range(config["episode_length"]):
            ...
            for t in range(len(torch_obs)):
                torch_obs[t] = torch_obs[t].T
            ...
            actions = np.array([[ac[i] for ac in agent_actions] for i in range(config["n_rollout_threads"])])[0]
